...
//When we learn about the arrays, we can ask the user for the size.
const int width = 5;
const int height = 8;
...
const int UNFLAGGED_MINE = -1; //The cell is a mine that has not been flagged yet
...
//These three variables shows the state of games. You have to change these states
// during the game such the values reflect the correct state of the game.
...
//To write a complex program your code needs to be modular. This means that we break down the program into functions.
//Each function is responsible for one part of the functionality. Here is the list of the functions in our program. 
...
void setMine(int num_of_mine) {
	...
	while (current_mine < num_of_mine) {
		...
		//By using % operator, we can make sure that we generate random number between 0 and height/width (excluding height/width). 
		...
		if ( getCell(randRow,randCol) != UNFLAGGED_MINE) {
			setCell(randRow, randCol, UNFLAGGED_MINE);
			current_mine++;
		}
	}
}
...
//The initField function initilizes the field with UNKOWN cells and then randomly sets some of the cells to be mines (using the setMine function)
//Input:
//	num_of_mine: The number of mines in the field
//Output:
//	None	
void initField(int num_of_mine) {//initializes feild with unknown characters and sets number of mine 
    for (int i = 0; i < 8; i++){
        for ( int j = 0; j < 5; j++){
            setCell(i,j,UNKNOWN);
        }
...
bool isMine(int i, int j) {//checks if a cell is mine or not 
...
void reveal(int i,int j) {//checks the surrounding 8 cells from the user command and increments the adjacent mine to neccesary values 
	int adjacent_mine = 0;
	//top left
	...
	//top
	if(i - 1 >= 0 && j >= 0 &&  isMine(i - 1,j)){ 
		adjacent_mine += 1;
	} 

	//left
	if(i >= 0 && j - 1 >= 0 && isMine(i,j-1)){
		adjacent_mine += 1;
	}

	//bottom left
	if(i + 1 < 8 && j - 1 >= 0 && isMine(i+1,j-1)){
		adjacent_mine += 1;
	}

	//top right
	if(i - 1 >= 0 && j + 1 < 5 && isMine(i-1,j+1)){
		adjacent_mine += 1;
	}

	//right
	if(i >= 0 && j + 1 < 5 && isMine(i,j+1)){
		adjacent_mine += 1;
	}

	//bottom right
	if(i + 1 < 8 && j + 1 < 5 && isMine(i+1,j+1)){
		adjacent_mine += 1;
	}

	//bottom
	if(i + 1 < 8 && j >= 0 && isMine(i+1,j)){
		adjacent_mine += 1;
	}
	

	setCell(i,j,adjacent_mine);
}
...
//	None  
void executeCmd(string cmd) {
	//getting row and column indeces from user command and converting ascii index to origanal number 
	int row_id = cmd.at(1) - 48;
	int col_id = cmd.at(2) - 48;

	//reveal 
	 if(cmd.at(0) == 'r'){//checks for user command r, if mine then game is over, if not mine then reveal is called 

		if(getCell(row_id,col_id) == UNFLAGGED_MINE || getCell(row_id,col_id) == FLAGGED_MINE ){
			explode = true;
		}

		else  {
			reveal(row_id,col_id);
		}
	 }
	 

	 //flag mine 
	 if (cmd.at(0) == 'f'){//checks for user command f, if unflagged mine then its set to flagged mine, otherwise incorrect flagged mine 

		if(getCell(row_id,col_id) == UNFLAGGED_MINE){
			setCell(row_id,col_id,FLAGGED_MINE);
			num_correct_flagged_mine += 1;
		}

		else{
			setCell(row_id,col_id, INCORRECT_FLAGGED_MINE);
	 ...
	 //undoes the flag mine 
	 if(cmd.at(0) == 'u'){//checks for user command u, if flagged mine then its set to an unflagged mine, if incorrect flagged mine then its set to unknown

		if(getCell(row_id,col_id) == FLAGGED_MINE){
			setCell(row_id,col_id,UNFLAGGED_MINE);
			num_correct_flagged_mine -= 1;
		}

		else if(getCell(row_id,col_id) == INCORRECT_FLAGGED_MINE){
			setCell(row_id,col_id,UNKNOWN);
			num_incorrect_flagged_mine = -1;
		}
